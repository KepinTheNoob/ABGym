...
export default function Members() {
  ...
  const createMemberMutation = useMutation({
    mutationFn: async (payload: any) => {
      console.log("Creating/Renewing member with payload:", payload);
  ...
  const handleRenewalSubmit = (data: any) => {
    createMemberMutation.mutate({
       ...
       // You probably want to add: id: selectedMemberId 
    });
  };
    ...
    <div className="min-h-screen bg-[#0c0c0e] text-white flex">
      ...
      <main className="flex-1 p-4 md:p-8 overflow-y-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
           ...
           {/* I kept this collapsed for brevity since it didn't change */}
                    ...
                    <tr key={member.id} className="border-b border-gray-800/50 hover:bg-gray-900/30">
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          <img src={member.avatar} className="w-10 h-10 rounded-full border border-gray-700" />
                          <div>
